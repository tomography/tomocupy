...
class Tests(unittest.TestCase):
    ...
    def test_recon_step(self):
        os.system('rm -rf data_rec')
        cmd = 'tomocupy recon_steps --file-name data/test_data.h5 --reconstruction-type full --rotation-axis 770'
        print(f'TEST {inspect.stack()[0][3]}: {cmd}')
        st = os.system(cmd)
        self.assertEqual(st, 0)
        ssum = np.sum(dxchange.read_tiff_stack(
    ...
    def test_recon_step_retrieve_phase(self):
        os.system('rm -rf data_rec')
        cmd = 'tomocupy recon_steps --file-name data/test_data.h5 --reconstruction-type full --rotation-axis 770 --retrieve-phase-method paganin --retrieve-phase-alpha 0.0001'
        print(f'TEST {inspect.stack()[0][3]}: {cmd}')
        st = os.system(cmd)
        self.assertEqual(st, 0)
        ssum = np.sum(dxchange.read_tiff_stack(
    ...
    def test_recon_step_retrieve_phase_f16(self):
        os.system('rm -rf data_rec')
        cmd = 'tomocupy recon_steps --file-name data/test_data.h5 --reconstruction-type full --rotation-axis 770 --retrieve-phase-method paganin --retrieve-phase-alpha 0.0001 --dtype float16'
        print(f'TEST {inspect.stack()[0][3]}: {cmd}')
        st = os.system(cmd)
        self.assertEqual(st, 0)
        ssum = np.sum(dxchange.read_tiff_stack(
    ...
    def test_recon_step_rotation(self):
        os.system('rm -rf data_rec')
        cmd = 'tomocupy recon_steps --file-name data/test_data.h5 --reconstruction-type full --rotation-axis-auto auto  --rotation-axis-pairs [0,719]'
        print(f'TEST {inspect.stack()[0][3]}: {cmd}')
        st = os.system(cmd)
        self.assertEqual(st, 0)
        ssum = np.sum(dxchange.read_tiff_stack(
    ...
    def test_full_recon_crop(self):
        os.system('rm -rf data_rec')
        cmd = 'tomocupy recon --file-name data/test_data.h5 --reconstruction-type full --crop 512'
        print(f'TEST {inspect.stack()[0][3]}: {cmd}')
        st = os.system(cmd)
        self.assertEqual(st, 0)
        data = dxchange.read_tiff_stack(
