...
for iz in {0..5}; do
    for ir in {0..2}; do
        ...
        sleep 5
        echo ${size[$iz]} ${nz[$iz]} 'linerec' 'float32'
        tomocupy recon --file-name /local/data/tmp${size[$iz]}.h5 --reconstruction-type full --dtype float32 --reconstruction-algorithm linerec --max-write-threads 8 --nsino-per-chunk ${nz[$iz]}
        cp time.npy times/time_${size[$iz]}_${nz[$iz]}_linerec_float32_$ir.npy                
