...
fp16 = dxchange.read_tiff('res/linefp32.tiff').astype('float32')#*0.94
fp32 = dxchange.read_tiff('res/fp32.tiff').astype('float32')

fp16 = np.roll(fp16,1,axis=0)

# fp16 = fp16*np.linalg.norm(fp32)/np.linalg.norm(fp16)
fig = plt.figure(constrained_layout=True, figsize=(10.7, 5.6))
grid = fig.add_gridspec(2, 3, height_ratios=[1])
...
plt.colorbar(im, cax=cax,format='%.0e')
plt.savefig('difference_fourier_line.png',dpi=300,bbox_inches='tight')