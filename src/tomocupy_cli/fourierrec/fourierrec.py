...
class FourierRec():
    def __init__(self, n, nproj, nz, theta, dtype):                 
        ...
        self.nproj = nproj
        if dtype=='float16':
            self.fslv = cfunc_fourierrecfp16.cfunc_fourierrec(nproj, nz//2, n, self.theta.data.ptr)            
        else:
            self.fslv = cfunc_fourierrec.cfunc_fourierrec(nproj, nz//2, n, self.theta.data.ptr)
        
        

    def backprojection(self, obj, data, stream): 
        # reorganize data as a complex array, reuse data
        data = cp.ascontiguousarray(cp.concatenate((data[:self.nz//2,:,:,cp.newaxis],data[self.nz//2:,:,:,cp.newaxis]),axis=3).reshape(data.shape))
        # reuse obj array
