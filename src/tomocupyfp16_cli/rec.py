...
class GPURec():
    ...
    def darkflat_correction(self, data, dark, flat):
        ...
        dark0 = cp.mean(dark.astype('float16'), axis=0)        
        flat0 = cp.mean(flat.astype('float16'), axis=0)
    ...
    def recon(self, obj, data, dark, flat):
        ...
        # print(f'{cp.linalg.norm(obj.astype("float32"))=}')
    ...
    def recon_all(self):
        ...
        item_pinned['data'] = utils.pinned_array(
            np.zeros([2, self.nproj, self.nz, self.ni], dtype='float16'))
        item_pinned['dark'] = utils.pinned_array(
            np.zeros([2, self.ndark, self.nz, self.ni], dtype='float16'))
        item_pinned['flat'] = utils.pinned_array(
            np.ones([2, self.nflat, self.nz, self.ni], dtype='float16'))

        # gpu memory for data item
        item_gpu = {}
        item_gpu['data'] = cp.zeros(
            [2, self.nproj, self.nz, self.ni], dtype='float16')
        item_gpu['dark'] = cp.zeros(
            [2, self.ndark, self.nz, self.ni], dtype='float16')
        item_gpu['flat'] = cp.ones(
            [2, self.nflat, self.nz, self.ni], dtype='float16')

        # pinned memory for reconstrution
        rec_pinned = utils.pinned_array(
            np.zeros([2, self.nz, self.n, self.n], dtype='float16'))
        # gpu memory for reconstrution
        rec = cp.zeros([2, self.nz, self.n, self.n], dtype='float16')
    ...
    def recon_all_try(self):
        ...
        dark = dark[:, idslice:idslice+2**self.args.binning].astype('float16')
        flat = flat[:, idslice:idslice+2**self.args.binning].astype('float16')
        ...
        rec = cp.zeros([self.nz, self.n, self.n], dtype='float16')
